...
const EN: PanelCopy = {
  ...
  settings: {
    ...
    unrestrictedBrowserAccessHelp: 'Let the model read every HTTP(S) page, inspect all open tab titles and URLs, and perform actions—including following and closing tabs—without confirmation. Browser-protected pages remain unreadable.',
...
const ZH: PanelCopy = {
  ...
  settings: {
    ...
    unrestrictedBrowserAccessHelp: '模型无需确认即可读取所有 HTTP(S) 页面、查看全部已打开标签页的标题和链接，并执行包括跟随、关闭标签页在内的所有操作。浏览器受保护页面仍不可读取。',
