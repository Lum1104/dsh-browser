...
**分离的通道**：浏览器页面以结构化文本呈现（带编号的交互元素清单）；只有模型显式调用 `browser_screenshot` / `browser_read_image` 时才截取像素——按「页面读取」走审批，且要求宿主挂载附件存储、当前模型支持图片输入。另一方面，dsh 0.1.1 宿主可声明多模态图片限制，侧栏据此接收 PNG、JPEG、WebP 和 GIF，并渲染会话中的持久图片附件。
...
| 等待 | `browser_wait` | 页面加载与渲染稳定检测；也可阻塞至指定文本/选择器出现或消失 |
| 定位控件 | `browser_find` | 按文本、可访问名、角色或选择器定位，返回可操作编号，无需完整快照 |
| 批量操作 | `browser_act` | 一次往返内最多 12 个步骤（type/click/press/hover/select/scroll/wait） |
| 下拉选择 | `browser_select_option` | 按选项文本或 value 选择，支持多选 |
| 悬停 | `browser_hover` | 发送 pointer + mouse 事件序列，触发悬停菜单与提示 |
| 看页面 | `browser_screenshot` | 视口或单个元素，经 `chrome.tabs.captureVisibleTab` 截取并缩放到宿主预算 |
| 读图片 | `browser_read_image` | 图片自身字节（用扩展的 host 权限抓取）、canvas/video 当前帧，或元素区域裁剪 |
| 管理标签页 | `browser_tabs` | 列出、新开、切换控制权、关闭。切换控制权时会把用户当前标签页标记为「保留」，不会悄悄劫持视图 |
| 图片对话 | `session.prompt` / `session.attachment` | 按宿主能力启用图片选择、纯图片发送和持久历史预览 |
| 展开隐藏内容 | `browser_expand` | 点击折叠控件并滚动加载懒内容，每轮有上限；用较宽的否定名单挡掉任何破坏性/交易性按钮 |
| 联网搜索 | `browser_search` | 在后台标签页发起查询，链接采用通用抓取（不依赖各搜索引擎的 CSS 结构），用完即关 |
| 批量读网页 | `browser_read_pages` | 最多 8 个后台标签页、并发 3，按请求顺序汇总；单页失败只变成一个小节，不会让整次调用失败 |
| 下载文件 | `browser_download` / `browser_downloads` | 由扩展作为发起方调用 `chrome.downloads`：不触发页面级多文件下载弹窗，可指定文件名/子目录，并按 id 查看与控制进度 |
| 人机验证 | `browser_verify` | 页面提供控件几何位置，`chrome.debugger` 的 `Input.dispatchMouseEvent` 发出可信点击；权限为可选项，仅在该次点击前后挂载与断开 |
| 切换代理预设 | 侧边栏 | 选择会话运行的 preset；会话已有历史时，改为用该预设新建会话 |
